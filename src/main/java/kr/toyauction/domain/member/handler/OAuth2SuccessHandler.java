...
public class OAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException {
        ...
        Optional<Member> member = memberRepository.findByPlatformId((String) oAuth2User.getAttributes().get("email")); // 메일로 db 검색 추후 타 플랫폼이 추가 되면 검색 조건에 플랫폼까지 추가해서 검색 해야함
        ...
        if (member.isEmpty()) {
            ...
            memberRepository.save(saveMember);  // 신규 멤버 저장
        }
        Member getMember = memberRepository.findByPlatformId((String) oAuth2User.getAttributes().get("email"))
                .orElseThrow(DomainNotFoundException::new);
        String token = jwtProvider.createToken(getMember); // 토큰 발행
